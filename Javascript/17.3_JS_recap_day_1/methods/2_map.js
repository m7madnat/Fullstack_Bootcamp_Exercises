...
// variable.
const toNum = numbers.map((element) => {
  return +element;
});
...
// Loop over this array with the map method that will produce a new array
// and change the string numbers to actual float numbers. Store the result in a// variable.
const toNum2 = floatNumbers.map((element) => {
...
const movieResault = movies.map((e) => {
  return e.name;
});
...
const arr = [1, 2, 3];
const [second, first, ...res] = arr;
// console.log(first);

const obj = { name: 'foo', last: 'bar' };
const { last, name } = obj;
// console.log(last);
