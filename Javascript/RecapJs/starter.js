...
// console.log(getAgencyByName(ourCarMarket.sellers, 'Best Deal'));
...
const getAllAgenciesNameAndId = (agenciesArr) => {  
    return agenciesArr.map(agency => ({name: agency.agencyName, id: agency.agencyId})); 
};
...
const getCarsToBuy = (agenciesArr) => {
  // const allCarsToBuy = [];  //first solution
  // for (let i = 0; i < agenciesArr.length; i++) {  
  //   for (const car in agenciesArr[i].cars) {
  //     allCarsToBuy.push(...agenciesArr[i].cars[car]);
  //   }
  // }
  // return allCarsToBuy;
  
  agenciesArr.forEach(agency => { // agency = اسم متغير ونستطيع تغييره كما نريد  second solution
    const cars = agency.cars;
    for(let carBrand in cars) { 
      console.log(cars[carBrand]);
    }    
    // for(let carsArr of Object.values(cars)) {  //third solution
    //   console.log(carsArr);
    // }
...
const getCarsToBuyByModel = (agenciesArr, model) => {
  const allCarsToBuy = [];
  for (let i = 0; i < agenciesArr.sellers.length; i++) {  
    for (const car in agenciesArr.sellers[i].cars) {
      allCarsToBuy.push(...agenciesArr.sellers[i].cars[car]);
    }
  }
  return allCarsToBuy.filter(car => car.name === model); // used filter to get all cars with the same model
};
// console.log(getCarsToBuyByModel(ourCarMarket, 'Hilux'));
...
// console.log(getCarsToBuyByAgencyId(ourCarMarket.sellers, 'oqQmsZoUo'));
...
const getCarsToBuyByModelAndAgencyId = (agenciesArr, model, agencyId) => { 
  ...
  agenciesArr.forEach((agency) => {
    ...
    for (let carBrand in cars) {
      cars[carBrand].forEach((car) => {
        if (agency.agencyId === agencyId && car.name === model) {
          res.push(car);
        } 
      });
    }
...
// console.log(getCarsToBuyByModelAndAgencyId(ourCarMarket.sellers, 'X5', "26_IPfHU1"));
...
const getCustomerByName = (customersArr, name) => {  
  return customersArr.customers.find(customer => customer.name === name);
};

// console.log(getCustomerByName(ourCarMarket, 'Ravi Murillo'));
...
const getCustomerById = (customersArr, id) => {
  return customersArr.customers.find(customer => customer.id === id);
};
// console.log(getCustomerById(ourCarMarket, '2RprZ1dbL'));
...
const getAllCustomersCars = (customersArr) => {
  const customerCarsArr = [];
  for (let i = 0; i < customersArr.customers.length; i++) {
    customerCarsArr.push(...customersArr.customers[i].cars);
  }
  return customerCarsArr;  
};
// console.log(getAllCustomersCars(ourCarMarket));
...
const setNewCarToAgency = (agencyObject, carObject) => {
  ...
  const brand = carObject.brand;
  agencyObject.cars[brand] = agencyObject.cars[brand] || [];
  agencyObject.cars[brand].push(carObject);
  return agencyObject;
  
}

// console.log(setNewCarToAgency(ourCarMarket.sellers[0], {brand: 'Opel' ,name: 'insigna',year: 2010, price: 10000, carNumber: '123456',ownerId: '2RprZ1dbL'}));
...
const deleteCarFromAgency = (agencyObject, carNumber) => {
  ...
  for (let carBrand in agencyObject.cars) {
    agencyObject.cars[carBrand] = agencyObject.cars[carBrand].filter(car => car.carNumber !== carNumber);
  }
  return agencyObject;
};
// console.log(deleteCarFromAgency(ourCarMarket.sellers[0], "4Ixb0"));
...
// console.log(decOrIncCashOfAgency(ourCarMarket.sellers[0], 32000));
...
const decOrIncCreditOfAgency = (agencyObj, amount) => {
  ...
  if (agencyObj.credit < 0) {
    agencyObj.credit = 0;
  }
  return agencyObj.credit;
};
// console.log(decOrIncCreditOfAgency(ourCarMarket.sellers[0], -200000));
...
// console.log(setCarToCustomer(ourCarMarket.customers[0], {brand: 'Opel' ,name: 'insigna',year: 2010, price: 10000, carNumber: '123456',ownerId: '2RprZ1dbL'}));
...
const deleteCarOfCostumer = (costumerObj, carNumber) => {
  ...
  costumerObj.cars = costumerObj.cars.filter(car => car.carNumber !== carNumber);
...
const decOrIncCashOfCustomer = (costumerObj, amount) => {
    ...
    if (costumerObj.cash < 0) {
      costumerObj.cash = 0;
    }
    return costumerObj.cash;
};
// console.log(decOrIncCashOfCustomer(ourCarMarket.customers[0], +200000));
